...
total_cat_in_shapenet_list = ['table', 'jar', 'skateboard', 'car', 'bottle', 'tower', 'chair', 'bookshelf', 'camera', 'airplane', # 正しくないShaneNetCatリスト作る
'laptop', 'basket', 'sofa', 'knife', 'can', 'rifle', 'train', 'lamp', 'trash bin', 'mailbox', 
...
'bus', 'mug', 'remote', 'bathtub', 'bowl', 'keyboard', 'guitar', 'washer', 'bicycle', 'faucet', 'printer', 'cap', 'trash_bin', 'tv or monitor'] # 'pillow', 'box', 
...
error_cat_list = {
    'total': [
        ...
        'jacket', 'cup', 'books', 'yoga mat', 'helmet', 'shoes', 'bag', 'pitcher', 'speaker', 'object', 'pillow', 'box', 'person', 'purse', 
        'mouse', 'shorts', 'clothing', 'clothes', 'doorframe', 'trash can', 'duffel bag', 'whiteboard', 'paper bag', 'radiator', 'board', 
        'breakfast bar', 'file cabinet', 'shelf', 'cabinet', 'footrest', # Chair用 'shelf', 'cabinet', 
    ]
}

ok_cat_list = {
    'table': ['nightstand'], 
    'chair': ['toilet', 'stool', 'luggage stand', 'beanbag chair'], 
    'display': [], 
    'cabinet': ['kitchen cabinet', 'refrigerator', 'dresser', 'nightstand', 'mini fridge', 'closet wardrobe', 'wardrobe'], 
}
...
if sys.argv[-3] == 'chair':
    ...
    target_shapenet_cat = {'04379243': 'table', }
elif sys.argv[-3] == 'display':
    target_shapenet_cat = {'03211117': 'display', }
elif sys.argv[-3] == 'cabinet':
    target_shapenet_cat = {'02933112': 'cabinet', }
...
for target_scan_id in target_scan_list:
    ...
    for idx, (instance_id, _) in enumerate(annotations_dict[target_scan_id]['cad'].items()):
        ...
        for list_idx, scannet_instance_idx in enumerate(sorted_near_instance_idx_list):
            ...
            print(f'{instance_id}:{target_shapenet_cat[catid]}:::{bbox_scannet_cat}:{bbox_shapenet_cat}')
            ...
            #    - match_another_shapecat    : 要確認
            #    - unmatched_init_one        : 要確認
            #    - match_one_not_in_shapecat : 要確認
            matched = False
            if bbox_shapenet_cat in total_cat_in_shapenet_list: # カテゴリがShapeNetに存在するか？
                ...
                if not matched:
                    if target_shapenet_cat[catid] == 'chair' and  bbox_shapenet_cat in {'table', 'tv or monitor', 'trash_bin', 'bookshelf', 'cabinet'}: # 
                        ... # 机と椅子を分離
                    else:
                        with open(os.path.join(out_dir, f'match_another_shapecat.txt'), 'a') as f:
            ...
            if matched:
                ...
                if list_idx == 1:
                    with open(os.path.join(out_dir, f'unmatch_1_times.txt'), 'a') as f:
                        ...
                        for past_matched_idx in past_past_matched_idx_list[:-1]:
                            print(f'     {scannet_cat_list[past_matched_idx]}:{shapenet_cat_list[past_matched_idx]}', file=f)
                elif list_idx == 2:
    ...
    check_map_np(get_colored_segmap(total_ins_mask[:, :, None]), 'manual_check_ins.png')
    check_map_np(total_rgb, 'manual_check_rgb.png')
    total_ids = np.unique(total_ins_mask).tolist()
    for ids_i in total_ids:
        str_id = str(ids_i)
        check_map_np(total_ins_mask==ids_i, f'manual_check_{str_id}.png')
    import pdb; pdb.set_trace()
    
    # マニュアルで書き換え
    print('##### TARGET OBJ #####')
    print(annotations_dict[target_scan_id]['cad'].keys())
    # 例：obj_id = '03001627_9dfaa9b599fced446f1e07a56c129dfc_006'
    # 例：annotations_dict[target_scan_id]['cad'][obj_id]['shapened_instance_id'] = ?

    # # scene0665_00 Table
    # obj_id = '03001627_9dfaa9b599fced446f1e07a56c129dfc_006'
    # annotations_dict[target_scan_id]['cad'][obj_id]['shapened_instance_id'] = 22
    # obj_id = '03001627_b58b8a4646d14b7c202339ec5396045d_001'
    # annotations_dict[target_scan_id]['cad'][obj_id]['shapened_instance_id'] = 30
    # obj_id = '03001627_b58b8a4646d14b7c202339ec5396045d_002'
    # annotations_dict[target_scan_id]['cad'][obj_id]['shapened_instance_id'] = 32
    # obj_id = '03001627_b58b8a4646d14b7c202339ec5396045d_003'
    # annotations_dict[target_scan_id]['cad'][obj_id]['shapened_instance_id'] = 3
    
    # # scene0155_02 Table
    # obj_id = '03001627_b58b8a4646d14b7c202339ec5396045d_001'
    # annotations_dict[target_scan_id]['cad'][obj_id]['shapened_instance_id'] = 17
    # obj_id = '03001627_b58b8a4646d14b7c202339ec5396045d_002'
    # annotations_dict[target_scan_id]['cad'][obj_id]['shapened_instance_id'] = 16

    # scene0489_02 Table
    obj_id = '03001627_b58b8a4646d14b7c202339ec5396045d_001'
    annotations_dict[target_scan_id]['cad'][obj_id]['shapened_instance_id'] = 12
    ...
    for img_cnt, img_id in enumerate(tqdm.tqdm(range(0, MAX_IM_NUM, 10))): # 0始まり
        ...
        total_depth = np.frombuffer(depth_data, dtype=np.uint16).reshape(depth_height, depth_width) / 1000 # np.fromstring(depth_data, dtype=np.uint16).reshape(depth_height, depth_width) / 1000
        total_rgb = frame_infos.decompress_color(color_compression_type)
        pose = frame_infos.camera_to_world
        total_ins_mask = cv2.imread(os.path.join(SCANNET_DIR, target_scan_id, f'instance-filt/{img_id}.png'))[:, :, 0]
        if not os.path.isfile(os.path.join(SCANNET_DIR, target_scan_id, f'instance-filt/{img_id}.png')):
            continue
        # # データの読み込み
