...
def export(mesh_file, agg_file, seg_file, meta_file, label_map_file, output_file=None):
    ...
    rawcat2shapenet = scannet_utils.read_label_mapping(label_map_file,
        label_from='raw_category', label_to='ShapeNetCore55')     
    ...
    shapenet_cats = np.zeros(shape=(num_verts), dtype=object) # 0: ShapeNetのカテゴリ取得のため追記
    scannet_cats = np.zeros(shape=(num_verts), dtype=object) # 0: ShapeNetのカテゴリ取得のため追記
    object_id_to_label_id = {}
    object_id_to_shapenet_cat = {} # 0: ShapeNetのカテゴリ取得のため追記
    object_id_to_scannet_cat = {} # 0: ShapeNetのカテゴリ取得のため追記
    for label, segs in label_to_segs.items():
        label_id = label_map[label]
        shapenet_id = rawcat2shapenet[label] # 0: ShapeNetのカテゴリ取得のため追記
        for seg in segs:
            verts = seg_to_verts[seg]
            label_ids[verts] = label_id
            shapenet_cats[verts] = shapenet_id # 0: ShapeNetのカテゴリ取得のため追記
            scannet_cats[verts] = label # 0: ShapeNetのカテゴリ取得のため追記
    ...
    for object_id, segs in object_id_to_segs.items():
        for seg in segs:
            ...
            if object_id not in object_id_to_label_id:
                ...
                object_id_to_shapenet_cat[object_id] = shapenet_cats[verts][0] # 0: ShapeNetのカテゴリ取得のため追記
                object_id_to_scannet_cat[object_id] = scannet_cats[verts][0] # 0: ShapeNetのカテゴリ取得のため追記
    instance_bboxes = np.zeros((num_instances,7))
    scan_net_labels = ['zero_pt'] * num_instances
    shape_net_labels = ['zero_pt'] * num_instances
    for obj_id in object_id_to_segs:
        ...
        shape_net_labels[obj_id-1] = object_id_to_shapenet_cat[obj_id] # 0: ShapeNetのカテゴリ取得のため追記
        scan_net_labels[obj_id-1] = object_id_to_scannet_cat[obj_id] # 0: ShapeNetのカテゴリ取得のため追記
    ...
    return mesh_vertices, label_ids, instance_ids,\
        instance_bboxes, object_id_to_label_id, shape_net_labels, scan_net_labels
