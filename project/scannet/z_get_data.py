...
error_cat_list = {
    'total': [
        ...
        'jacket', 'cup', 'books', 'yoga mat', 'helmet', 'shoes', 'bag', 'pitcher', 'speaker', 'object', 'pillow', 'box', 
    ]
}
    ...
    for idx, (instance_id, _) in enumerate(annotations_dict[target_scan_id]['cad'].items()):
        ...
        for list_idx, scannet_instance_idx in enumerate(sorted_near_instance_idx_list):
            ...
            if bbox_shapenet_cat in total_cat_in_shapenet_list: # カテゴリがShapeNetに存在するか？
                ...
                if not matched:
                    if target_shapenet_cat[catid] == 'chair' and  bbox_shapenet_cat == 'table':
                        ... # 机と椅子を分離
                    else:
        ...
        for target_instance_id in annotations_dict[target_scan_id]['cad'].keys():
            ...
            if (total_ids==instance_label).any():
                ...
                if enough_bbox_size and have_distance:
                    ...
                    vis_d_masked = deepcopy(vis_d)
                    vis_d_masked[~mask] = 0.

                    from segmentation_color import get_colored_segmap
                    total_ins_mask_img =  cv2.resize(get_colored_segmap(total_ins_mask[:, :, None])[..., [2, 1, 0]], (int(W/H*clopped_H), clopped_H))
                    total_rgb_img = cv2.resize(total_rgb[..., [2, 1, 0]], (int(W/H*clopped_H), clopped_H))
                    
                    buffer = 15
                    total_check_img = np.ones((clopped_H, clopped_H*3+int(W/H*clopped_H)*2+buffer*4, 3)).astype(np.uint8) * 255
                    total_check_img[:, :int(W/H*clopped_H)] = total_rgb_img
                    total_check_img[:, int(W/H*clopped_H)+buffer:int(W/H*clopped_H)*2+buffer] = total_ins_mask_img
                    total_check_img[:, int(W/H*clopped_H)*2+2*buffer:int(W/H*clopped_H)*2+clopped_H+buffer*2] = rgb[..., [2, 1, 0]]
                    total_check_img[:, int(W/H*clopped_H)*2+clopped_H+3*buffer:int(W/H*clopped_H)*2+clopped_H*2+buffer*3] = vis_d
                    total_check_img[:, int(W/H*clopped_H)*2+clopped_H*2+4*buffer:] = vis_d_masked
                    cv2.imwrite('realdata_sample.png', total_check_img)
                    import pdb; pdb.set_trace()
