...
class original_optimizer(pl.LightningModule):
    ...
    def __init__(self, args, pretrain_models):
        ...
        self.rs = {'train': np.random.RandomState(78)}
    ...
    def preprocess(self, batch, mode):
        ...
        # print(rand_P_seed[:, 0])
        # print(rand_S_seed[:, 0])
        # print(randn_axis_idx)
        # print(randn_theta_seed)
    ...
    def forward(self, batch_size, current_frame_num, clopped_mask, clopped_distance_map, past_itr_log, pre_obj_pos_wrd, 
                ...
                rays_d_cam, batch_idx, optim_idx, print_debug, model_mode):
        ...
        self.df_net(obs_embed, est_embed, dif_embed, tra_embed, past_itr_log['itr_length'], optim_idx, inp_pre_obj_pos_wrd, inp_pre_obj_axis_green_wrd, inp_pre_obj_axis_red_wrd, inp_pre_o2w, \
                    inp_pre_obj_scale_wrd, inp_pre_shape_code, pre_obj_pos_wrd, pre_obj_axis_green_wrd, pre_obj_axis_red_wrd, pre_o2w, pre_obj_scale_wrd, pre_shape_code, print_debug) #, self.backbone_decoder)
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            frame_gt_obj_green_wrd, frame_gt_obj_red_wrd, frame_camera_pos_wrd, frame_gt_obj_pos_wrd, frame_gt_obj_scale_wrd, \
            ini_obj_pos, ini_obj_scale, ini_obj_axis_red_wrd, ini_obj_axis_green_wrd, ini_shape_code, frame_gt_o2w, instance_id, scene_id = self.preprocess(batch, mode='train')
        ...
        for optim_idx in range(self.total_itr):
            ...
            est_obj_pos_wrd, est_obj_axis_green_wrd, est_obj_axis_red_wrd, est_obj_scale, est_shape_code = \
                ...
                pre_obj_axis_red_wrd, pre_obj_scale_wrd, pre_shape_code, w2c, cam_pos_wrd, rays_d_cam, batch_idx, optim_idx, print_debug, 'train')
    ...
    def test_step(self, batch, batch_idx, step_mode='tes'):
        ...
        for optim_idx in range(-1, self.total_itr):
            ...
            ###################################
                ...
                est_obj_pos_wrd, est_obj_axis_green_wrd, est_obj_axis_red_wrd, est_obj_scale, est_shape_code = \
                    self.forward(batch_size, current_frame_num, clopped_mask, clopped_distance_map, past_itr_log, pre_obj_pos_wrd, pre_obj_axis_green_wrd, 
                    pre_obj_axis_red_wrd, pre_obj_scale_wrd, pre_shape_code, w2c, cam_pos_wrd, rays_d_cam, batch_idx, optim_idx, print_debug, 'val')
    ...
    def test_epoch_end(self, outputs):
        ...
        log_dict['gt_S_seed'] = np.concatenate([x['gt_S_seed'] for x in outputs])
...
if __name__=='__main__':
    ...
    if args.code_mode == 'TRAIN':
        ...
        val_dataset = TaR_dataset(args, 'val', args.val_instance_list_txt, args.val_data_dir, args.val_data_list)
