...
class original_optimizer(pl.LightningModule):
    ...
    def preprocess(self, batch, mode):
        ...
        if self.data_type == 'shapenet': # 書き換えた gt_model_3D_bbox_obj
            ...
            frame_gt_obj_scale_wrd, gt_model_3D_bbox_obj, canonical_distance_map, canonical_camera_pos, canonical_camera_rot, gt_pc_obj, rand_seed, sym_label, path, scene_id, gt_shape_code = batch
...
if __name__=='__main__':
    ...
    if args.code_mode == 'TRAIN':
        ...
        ##########################################################################################################################################################################################################################################################
        ##########################################################################################################################################################################################################################################################
        # CUDA_VISIBLE_DEVICES=7 python train_check_scannet.py --code_mode=TRAIN --config=configs/paper_exp/chair_re/view5/rdn.txt --exp_version=rdn_autoreg --N_batch=8 --pre_train_epoch=360 --data_type=scan2cad --save_interval=1
        # 書き換える
        ...
        train_dataset = scannet_dataset(args, 'train', args.scannet_data_list)
        ...
        trainer.fit(model=model, train_dataloaders=train_dataloader, val_dataloaders=val_dataloaders, datamodule=None, ckpt_path=ckpt_path)
