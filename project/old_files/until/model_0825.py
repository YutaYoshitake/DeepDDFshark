...
class optimize_former(pl.LightningModule):
    ...
    def __init__(
        ...
        elif self.split_into_patch in {'wPe', 'woPe'}:
            if not integration_mode == 'cls_token':
                sys.exit()
            self.split_map_size = 16
            if self.split_into_patch == 'wPe':
    ...
    def forward(self, inp, rays_d_cam, pre_scale_wrd, pre_shape_code, pre_o2w, 
        ...
        if self.split_into_patch == 'non':
            ...
            if self.split_into_patch == 'wPe':
                split_pos = torch.cat([
                    self.col_embed[None, None, :, :].repeat(seq_len, self.split_map_size, 1, 1), 
                    self.row_embed[None, :, None, :].repeat(seq_len, 1, self.split_map_size, 1), 
