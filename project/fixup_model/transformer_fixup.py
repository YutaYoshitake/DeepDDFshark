    ...
    def __init__(
        ...
        if T_Fixup:
            for encoder_layer in self.layers:
                encoder_layer.fixup_initialization(en_layers=T_Fixup_N)
        # =================================================================
        print(self.layers[0].fc1.weight[0, :5])
        print('##########     END     ##########')
...
class TransformerEncoderLayer_FixUp02(nn.Module):
    ...
    def forward(self, x, atten_mask=None, seq_padding_mask=None, itr_padding_mask=None):
        frame_atten_mask, itr_atten_mask = seq_padding_mask, itr_padding_mask

        residual = x
        x, self.multi_view_attn_output_weights = self.multi_view_attn(query=x, key=x, value=x, key_padding_mask=None, attn_mask=itr_atten_mask,)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x

        residual = x
        x, self.attn_output_weights = self.self_attn(query=x, key=x, value=x, key_padding_mask=None, attn_mask=frame_atten_mask,)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
        ...
        # x = F.dropout(x, p=self.dropout, training=self.training)
        # x = residual + x
        # x = x.reshape(seq, itr, batch, -1) # [seq, itr, batch, dim]
        ...
        # x = F.dropout(x, p=self.dropout, training=self.training)
        # x = residual + x
        # x = x.reshape(itr, seq, batch, -1) # [itr, seq, batch, dim]
