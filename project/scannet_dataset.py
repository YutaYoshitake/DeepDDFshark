...
class scannet_dataset(data.Dataset):
    def __init__(self, args, mode, raw_data_list):
        ...
        self.distmap_mask_type = 'use_gt_insmask'
    ...
    def __getitem__(self, index):
        ...
        # Eval point clouds.
        if self.mode in {'tes', 'val'}: # {'tes'}: # 
            gt_pc_obj = np.load(os.path.join(self.point_cloud_path, instance_id+'.npy')).astype(np.float32)
        else:
            gt_pc_obj = False

        # 深度観測にマスク
        if self.distmap_mask_type in {'use_gt_3DBbox', 'use_gt_insmask'}:
            distance_map[~mask] = 0.

        # # GTの3DBboxの内側のみ使う
        # if self.distmap_mask_type in {'use_gt_3DBbox'}:
        ...
        return mask, distance_map, instance_id, cam_pos_wrd, w2c, bbox_diagonal, bbox_list, rays_d_cam, obj_pos_wrd, o2w, \
            ...
            canonical_camera_pos, canonical_camera_rot, gt_pc_obj, rand_seed, sym_label, frame_path_list, [path[-24:-7] for path in frame_path_list], gt_shape_code, w2oriWrd, wrd_pos_oriWrd
...
# python scannet_dataset.py --config=./configs/paper_exp/chair_a/view5/txt.txt
if __name__=='__main__':
    ...
    for batch in dummy_dataloader:
        ...
        _, _, _, _, _, _, _, _, gt_shape_code, _, _ = batch
