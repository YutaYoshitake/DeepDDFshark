...
class df_resnet_encoder(pl.LightningModule):
    ...
    def forward(self, inp, bbox_info, pre_pos, pre_axis_green, pre_axis_red, pre_scale, pre_shape_code, h_0=False):
        ...
        # diff_scale_cim = torch.ones_like(pre_scale)

        # Get shape code diff.
        diff_shape_code = self.fc_shape_code(torch.cat([x, pre_shape_code], dim=-1))

        return diff_pos, diff_axis_green, diff_axis_red, diff_scale_cim, diff_shape_code, 0
...
class TaR(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for frame_sequence_idx, frame_idx in enumerate(frame_idx_list):
            ...
            for optim_idx in range(optim_num):
                ...
                if perform_init_est:
                    ...
                    est_obj_pos_cim, est_obj_axis_green_cam, est_obj_axis_red_cam, est_scale_cim, est_shape_code, pre_hidden_state = self.init_net(inp.detach(), bbox_info.detach())
                    ...
                    diff_pos_cim, diff_obj_axis_green_cam, diff_obj_axis_red_cam, diff_scale_cim, diff_shape_code, pre_hidden_state = self.df_net(
                                                                                                                                inp = inp.detach(), 
                                                                                                                                bbox_info = bbox_info.detach(), 
                                                                                                                                pre_pos = pre_obj_pos_cim.detach(), 
                                                                                                                                pre_axis_green = pre_obj_axis_green_cam.detach(), 
                                                                                                                                pre_axis_red = pre_obj_axis_red_cam.detach(), 
                                                                                                                                pre_scale = pre_obj_scale_cim.detach(), 
                ...
                # check_map_torch(check_map, f'frame{frame_idx}_optim{optim_idx}.png')
                ...
                if 0 < optim_idx and self.use_depth_error:
                    if self.depth_sampling_type == 'clopping':
                        ...
                        sampled_rays_d_cam = self.rays_d_cam.expand(batch_size, -1, -1, -1).to(w2c)
                    ...
                    loss_depth_i = self.l1(est_invdistance_map_for_deptherr, gt_invdistance_map_for_deptherr.detach())
                    # ##################################################
                    # # Check map.
                    # check_map = []
                    ...
                    #         break
                    # check_map = torch.cat(check_map, dim=0)
                    # check_map_torch(check_map, f'{batch_idx}.png')
                    # # import pdb; pdb.set_trace()
                    # ##################################################
                    # if torch.isnan(loss_depth_i):
                    #     check_map = []
                    #     for gt, est in zip(gt_invdistance_map_for_deptherr, est_invdistance_map_for_deptherr):
                    #         check_map.append(torch.cat([gt, est, torch.abs(gt-est)], dim=-1))
                    #         if len(check_map)>5:
                    #             break
                    #     check_map = torch.cat(check_map, dim=0)
                    #     check_map_torch(check_map, f'nan.png')
                    # ##################################################
                else:
                    loss_depth_i = torch.zeros(1).to(gt_obj_pos_wrd)
                depth_simulation_error.append(loss_depth_i)

                # Perform only initnet.
        ...
        #     self.isnan_list = [torch.isnan(params_i.grad).any().item() for params_i in opt.param_groups[1]['params']]
        #     self.isnan_list_0 = [torch.isnan(params_i).any().item() for params_i in opt.param_groups[0]['params']]
        #     if any(self.isnan_list):
        #         check_map_torch(torch.cat([est_invdistance_map_for_deptherr[0], gt_invdistance_map_for_deptherr[0]]), 'tes.png')
        #         import pdb; pdb.set_trace()
        #     opt.step()
        #     opt.zero_grad()
...
if __name__=='__main__':
    ...
    model = TaR(args, ddf)
    model.init_net = ini_model.init_net
    model.df_net = ini_model.df_net
