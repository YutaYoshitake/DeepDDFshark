...
# python quantitive.py --config=../configs/paper_exp/chair/view5/tes.txt --exp_version=subrndnsim_autoreg_tau01 --val_model_epoch=800
if __name__=='__main__':
    # 書き換える
    est_pickle_dir = '/home/yyoshitake/works/DeepSDF/project/paper_fig/normal_map/src/chair_re/2023_01_18_11_31_27_rdn_autoreg_epo_0000000128'
    ...
    test_log_name = args.model_ckpt_path.split('/')[-3] + '_epo_' + args.model_ckpt_path.split('/')[-1].split('.')[0]
    time_log = dt_now.strftime('%Y_%m_%d_%H_%M_%S') + '_' + test_log_name
    ...
    gto2c = gto2c[2][None, :, :].expand(freq, -1, -1)
    ...
    est_list = glob.glob(f'normal_map/src/{test_log_name}/*.pickle')
    ...
    for map_i, est_path in enumerate(tqdm.tqdm(est_list)):
        ...
        # obj_scale = est_scale # gt_scale / est_scale
        # cam_pos_wrd = torch.sum((cam_pos_gtobj - gtobj_pos_wrd)[..., None, :] * gt_o2w, dim=-1) * gt_scale
        # cam_pos_obj = cam_pos_gtobj # (torch.sum(cam_pos_wrd[..., None, :] * est_o2w.permute(0, 2, 1), dim=-1) + estobj_pos_wrd) / obj_scale # cam_pos_estobj
        ...
        # obj_pos_cam = torch.sum((gtobj_pos_wrd - cam_pos_wrd)[..., None, :] * w2c, dim=-1)
        # cam_pos_obj = - torch.sum(obj_pos_cam[..., None, :]*gt_o2c.permute(0, 2, 1), dim=-1) / gt_scale # cam_pos_gtobj # 
        # est_normal_map, est_distance_map, est_mask = render_normal_map_from_afar(H, gt_o2c, cam_pos_obj, gt_scale[:, 0], rays_d_cam, input_lat_vec, ddf)
        ...
        obs_map_path = sorted(glob.glob(f'depth_map/result/subrndnsim_autoreg_tau01_epo_0000000800/gt_{est_batch_idx}_*_00.png')) # sorted(glob.glob(f'depth_map/result/{test_log_name}/gt_{est_batch_idx}_*_00.png'))
