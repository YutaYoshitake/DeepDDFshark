...
if __name__=='__main__':
    ...
    # o2c = torch.roll(torch.flip(w2c, dims=[0]), 1, dims=0)
    # cam_pos_obj = cam_pos
    # rays_d_cam = rays_d_cam.expand(freq, -1, -1, -1)
    # obj_scale = torch.ones(freq)
    # for sample_idx in tqdm(range(1, 3001)):
    #     instance_id = random.sample(np.arange(ddf.lat_vecs.num_embeddings).tolist(), 2)
    #     instance_id = [388, 2532]
    ...
    #     cv2.imwrite('zzz.png', result_map[:, :, [2, 1, 0]])
