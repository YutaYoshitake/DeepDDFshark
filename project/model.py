...
class optimize_former(pl.LightningModule):
    ...
    def forward(self, obs_embed, est_embed, dif_embed, tra_embed, past_itr_length, optim_idx, inp_pre_obj_pos_wrd, 
        ...
        if self.add_conf == 'only_once':
            ...
            est_pos_wrd    = pre_pos_wrd.detach() + diff_pos_wrd                          # diff_pos_obj                         # pre_pos_wrd    # 
            est_green_wrd  = F.normalize(pre_green_wrd.detach() + diff_green_wrd, dim=-1) # F.normalize(diff_green_obj, dim=-1)  # pre_green_wrd  # 
            est_red_wrd    = F.normalize(pre_red_wrd.detach() + diff_red_wrd, dim=-1)     # F.normalize(diff_red_obj, dim=-1)    # pre_red_wrd    # 
            est_scale_wrd  = pre_scale_wrd.detach() * diff_scale_wrd                      # diff_scale_wrd                       # pre_scale_wrd  # 
            est_shape_code = pre_shape_code.detach() + diff_shape_code                    # diff_shape_code                      # pre_shape_code # 
